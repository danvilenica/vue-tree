...
import { useTraversal } from '../composables/treeViewTraversal.js'
...
const { depthFirstTraverse } = useTraversal(model);

const {
  focusableNodeModel,
  handleFocusableChange,
} = useTreeViewFocus(model);
...
onMounted(async () => {
  ...
  if (model.value.length > 0) {
    ...
    enforceSelectionMode(props.selectionMode);
  }
